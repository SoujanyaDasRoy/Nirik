...
def main():
    ...
    # Get first 10 TB images based on metadata
    import csv
    tb_files = []
    with open(metadata_path, 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row['findings'] == 'Tuberculosis':
    ...
    # Summary
    total = len(results)
    correct = sum(1 for r in results if r['prediction'] == 'Tuberculosis')
    ...
    print(f"Correct predictions (Tuberculosis): {correct}")
    ...
    print(f"Backend stable for TB cases? {'YES' if len(errors)==0 and incorrect==0 else 'NO'}")
