...
export default function DicomViewer({
  ...
  return (
    <div className="space-y-3 flex-1 flex flex-col">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border border-border/80 bg-muted/20 p-2.5 rounded-xl text-xs">
          ...
          <div className="flex items-center gap-1">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mr-1">Presets:</span>
            <div className="flex gap-1 bg-muted/30 p-0.5 rounded-full border border-border/40">
              {(["default", "lung", "bone", "softTissue"] as Preset[]).map(p => (
                <Button
                  key={p}
                  size="sm"
                  variant={activePreset === p ? "default" : "ghost"}
                  className={`h-8 px-3.5 rounded-full text-xs cursor-pointer capitalize transition-all duration-200 ${activePreset === p ? "bg-secondary text-secondary-foreground font-semibold shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
                  onClick={() => applyPreset(p)}
                >
                  {p === "default" ? "Default" : p.replace("softTissue", "Soft Tissue")}
                </Button>
              ))}
            </div>
          </div>
        ...
        {/* Row 2: Diagnostic Tools & Reset/Advanced Toggle */}
        <div className="flex flex-wrap items-center justify-between gap-3 border border-border/85 bg-muted/10 p-2.5 rounded-xl text-xs">
          {/* Primary Diagnostic Tools */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mr-1">Tools:</span>
            
            {/* Zoom Mode */}
            <Button
              size="sm"
              variant={zoomModeActive ? "default" : "outline"}
              className={`h-8 px-3.5 rounded-full text-xs cursor-pointer transition-all duration-200 ${zoomModeActive ? "bg-blue-600 hover:bg-blue-700 text-white font-semibold" : ""}`}
              onClick={() => {
            ...
            >
              <ZoomIn className="w-4 h-4 mr-1" /> Zoom Mode
            </Button>

            {/* Window/Level Mode */}
            <Button
              size="sm"
              variant={windowLevelModeActive ? "default" : "outline"}
              className={`h-8 px-3.5 rounded-full text-xs cursor-pointer transition-all duration-200 ${windowLevelModeActive ? "bg-purple-600 hover:bg-purple-700 text-white font-semibold" : ""}`}
              onClick={() => {
            ...
            >
              <Layers className="w-4 h-4 mr-1" /> Window/Level Drag
            </Button>

            {/* Ruler Measure */}
            <Button
              size="sm"
              variant={measureMode === "ruler" ? "default" : "outline"}
              className={`h-8 px-3.5 rounded-full text-xs cursor-pointer transition-all duration-200 ${measureMode === "ruler" ? "bg-yellow-500 hover:bg-yellow-600 text-black font-semibold animate-none" : ""}`}
              onClick={() => {
            ...
            >
              <RulerIcon className="w-4 h-4 mr-1" /> Ruler Measure
            </Button>

            {/* ROI Ellipse */}
            <Button
              size="sm"
              variant={measureMode === "roi" ? "default" : "outline"}
              className={`h-8 px-3.5 rounded-full text-xs cursor-pointer transition-all duration-200 ${measureMode === "roi" ? "bg-yellow-500 hover:bg-yellow-600 text-black font-semibold" : ""}`}
              onClick={() => {
                handleMeasureToggle(measureMode === "roi" ? "off" : "roi");
                setZoomModeActive(false);
                setWindowLevelModeActive(false);
                setAnnotateMode?.(false);
              }}
            >
              <Maximize2 className="w-4 h-4 mr-1" /> ROI Ellipse
            </Button>
            ...
            <Button
              ...
              className={`h-8 px-3.5 rounded-full text-xs cursor-pointer transition-all duration-200 ${annotateMode ? "bg-emerald-600 hover:bg-emerald-700 text-white font-semibold" : ""}`}
              onClick={() => {
            ...
            >
              <GitPullRequest className="w-4 h-4 mr-1" /> Draw Annotations
            </Button>

            {/* Active Drawing Zone Sub-Selector */}
            {annotateMode && (
              <div className="inline-flex items-center bg-muted/60 p-0.5 rounded-full border border-border/40 gap-0.5 ml-1">
                {(["Apical", "Mid-zone", "Basal", "Pleural"] as LungZone[]).map(zone => (
                  <button
                    key={zone}
                    type="button"
                    onClick={() => setActiveZone?.(zone)}
                    className={`text-[10px] font-semibold px-2.5 py-1 rounded-full transition-all border border-transparent cursor-pointer ${
                      activeZone === zone
                        ? "bg-background text-foreground shadow-sm font-bold border-border/20"
            ...
            {/* Lung Segments Toggle */}
            <Button
              size="sm"
              variant={lungSegmentationActive ? "default" : "outline"}
              className={`h-8 px-3.5 rounded-full text-xs cursor-pointer transition-all duration-200 ${lungSegmentationActive ? "bg-emerald-600 hover:bg-emerald-700 text-white font-semibold border-emerald-600" : ""}`}
              onClick={() => setLungSegmentationActive?.(!lungSegmentationActive)}
            >
              <Layers className="w-4 h-4 mr-1" /> Lung Segments
            </Button>

            {/* Fit Screen Options */}
            <div className="flex gap-0.5 bg-muted/40 p-0.5 rounded-full border border-border/50">
              <Button size="sm" variant="ghost" className="h-7 px-3 rounded-full text-[10px] cursor-pointer" onClick={() => adjustZoom('screen')}>Fit Screen</Button>
              <Button size="sm" variant="ghost" className="h-7 px-3 rounded-full text-[10px] cursor-pointer" onClick={() => adjustZoom('width')}>Width</Button>
              <Button size="sm" variant="ghost" className="h-7 px-3 rounded-full text-[10px] cursor-pointer" onClick={() => adjustZoom('height')}>Height</Button>
              <Button size="sm" variant="ghost" className="h-7 px-3 rounded-full text-[10px] cursor-pointer" onClick={() => adjustZoom('1to1')}>1:1</Button>
            </div>
          </div>

          {/* Reset and Expand Advanced */}
          <div className="flex items-center gap-1.5">
            <Button
              size="sm"
              variant={advancedToolsExpanded ? "default" : "outline"}
              className="h-8 px-3.5 rounded-full text-xs font-bold cursor-pointer"
              onClick={() => setAdvancedToolsExpanded(!advancedToolsExpanded)}
            >
              ⚙️ Advanced {advancedToolsExpanded ? "▲" : "▼"}
            </Button>
      ...
      {/* ── EXPANDABLE ADVANCED TOOLS CHEST ── */}
      {advancedToolsExpanded && (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 border border-border/70 p-3 rounded-xl bg-muted/10 text-xs animate-fadein">
          
          {/* Sliders */}
          <div className="md:col-span-8 grid grid-cols-3 gap-3">
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground font-mono font-bold flex justify-between">
                <span>Brightness</span> <span>{brightness.toFixed(2)}</span>
              </p>
              <Slider min={0.1} max={3} step={0.05} value={[brightness]} onValueChange={(v: any) => { setBrightness(v[0]); setActivePreset("default"); }} className="[&_[role=slider]]:size-3" />
            </div>
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground font-mono font-bold flex justify-between">
                <span>Contrast</span> <span>{contrast.toFixed(2)}</span>
              </p>
              <Slider min={0.1} max={4} step={0.05} value={[contrast]} onValueChange={(v: any) => { setContrast(v[0]); setActivePreset("default"); }} className="[&_[role=slider]]:size-3" />
            </div>
            <div className="space-y-1">
              <p className="text-[10px] text-muted-foreground font-mono font-bold flex justify-between">
                <span>Zoom Scale</span> <span>{zoom.toFixed(2)}×</span>
              </p>
              <Slider min={0.5} max={4} step={0.1} value={[zoom]} onValueChange={(v: any) => setZoom(v[0])} className="[&_[role=slider]]:size-3" />
            </div>
          </div>

          {/* Histogram */}
          <div className="md:col-span-4 flex flex-col gap-1.5 border-l border-border/80 pl-3">
            <p className="text-[8px] font-bold text-muted-foreground uppercase tracking-widest">Grayscale Distribution</p>
            <div className="w-full relative h-[32px] rounded border border-border/60 overflow-hidden bg-slate-950">
              <canvas ref={histCanvasRef} width={140} height={32} className="w-full h-full block" />
            </div>
          </div>

          {/* Advanced Toggles */}
          <div className="md:col-span-12 flex flex-wrap gap-1.5 border-t border-border/80 pt-2 items-center">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mr-1">Diagnostic Filters:</span>
            
            <Button size="sm" variant={edgeSharpen ? "default" : "outline"} className="h-8 px-3.5 rounded-full text-xs cursor-pointer" onClick={() => setEdgeSharpen(!edgeSharpen)}>
              <Sparkles className="w-3.5 h-3.5 mr-1" /> Sharpen Edges
            </Button>

            <Button size="sm" variant={detailBoost ? "default" : "outline"} className="h-8 px-3.5 rounded-full text-xs cursor-pointer" onClick={() => setDetailBoost(!detailBoost)}>
              <Zap className="w-3.5 h-3.5 mr-1" /> Detail Boost
            </Button>

            <Button
              size="sm"
              variant={invert ? "default" : "outline"}
              className="h-8 px-3.5 rounded-full text-xs cursor-pointer"
              onClick={() => setInvert(!invert)}
            >
              <RefreshCw className="w-3.5 h-3.5 mr-1" /> Invert Colors
            </Button>
          </div>
        </div>
      )}
